...
describe('App', () => {
  ...
  it('responds to requests on /', function (done) {
	...
	describe('users controller', () => {
		describe('list api', () => {
		    ...
		    request(TestApplication.app)
		      ...
		      .expect(200)
					.then(({ body }) => {
						...
						expect(users[0].name).to.be.eq('John Doe');
			...
			describe('pagination query', () => {
				it('responds with a limited result', done => {
					request(TestApplication.app)
						.get('/users?limit=2')
						.expect(200)
						.then(({ body }) => {
				...
				it('responds with 3 users when not given a limit', done => {
					request(TestApplication.app)
						.get('/users')
						.expect(200)
						.then(({ body }) => {
